...
TEST(RunSample, ValuesStored) {
    ...
    Sample result = run_sample(params, rng, n);
...
TEST(RunSample, AlreadyAtTargetSucceeds) {
    ...
    Sample sample = run_sample(params, rng, n, x0);
...
TEST(RunSamples, SuccessRateBounded) {
    ...
    DataPoint pt = run_samples(
        params, Q_pos, Q_vel, n_samples, n_threads, seed, n
    );
...
TEST(ComputeEnvelope, GridDimensions) {
    ...
    EnvelopeData env = compute_envelope(config, params, 2, n);
...
TEST(ComputeEnvelope, LogSpacingEndpoints)
{
    ...
    EnvelopeData env = compute_envelope(config, params, 1, n);
